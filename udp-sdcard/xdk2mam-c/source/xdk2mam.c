...
uint32_t parseIp(char* ip){
	int i=0,j=1,c=0;
	char *addr1=calloc(5, sizeof(char));
	char *addr2=calloc(5, sizeof(char));
	char *addr3=calloc(5, sizeof(char));
	char *addr4=calloc(5, sizeof(char));
	...
	return XDK_NETWORK_IPV4(atoi(addr1),
							atoi(addr2),
							atoi(addr3),
							atoi(addr4));
}
...
static void httpPostTask(void* parameter)
{
    ...
    char* udpBuffer;
    uint32_t IP = parseIp(DEST_SERVER_HOST);
	...
	while (1)
	{
		...
		udpBuffer = receiveBufferFromSensors();

		if (retcode == RETCODE_OK)
		{
			retcode = UDP_Send(handle,
							IP,
							(uint16_t)atoi(DEST_SERVER_PORT),
							(uint8_t*)&udpBuffer[0],
							strlen(udpBuffer));
			if (retcode == RETCODE_OK)
			{
